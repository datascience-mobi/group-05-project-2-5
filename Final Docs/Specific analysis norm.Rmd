...
## 3. Finding biomarkers {#anchor5}
<div style="text-align: justify">
Biomarkers are the genes which show the greatest response to the treatment. 
...
```
<div style="text-align: justify">
A lot of values are essentially zero, which would mean that one forth of all genes have showed significant responses to the treatment. This is ,however, dubious. 
...
```
<div style="text-align: justify">
For further analysis, we need to create a vector with the gene names of the p.Value Biomarker: 
</div>
```{r}
...
Then, we hypothesized that a decrease in variance would be more probable in cancerogenic genes, rather than cell line specific genes and therefore, selected the genes showing a negative variance change. 
...
# 1- Selecting genes with a variance above 95% quartile of variances before treatment
...
# 2 - Selecting genes which showed a significant decrease in their high variances
...
# 3.2- Create the heatmap

heatmap(Highest_VC_Biomarkers)
```

<div style="text-align: justify">
The heatmap shows that finding biomarkers using variance does not work as expected, since the cancer types do not cluster together.
</div>
...
```
<div style="text-align: justify">
Now we have determined biomarkers via the fold change. However, since we have worked only with absolute values, we cannot say whether those identified genes are mostly up- or downregulated by Vorinostat.
</div>
...
```
<div style="text-align: justify">
Now we know that p.Value and FC value do not deliver the same biomarkers. In fact, 100 out of 100 biomarkers are different.
If we compare the values directly, we see that a low p.value does not correlate with a high FC value. This is because small values are also assigned small p.values by the T-test and the result of the test is therefore falsified. We have also seen before that almost all genes were found to be significant in the T test, but this is not credible. 
...
FCbiomarkers.pca = prcomp(FCbiomarkers)
...
par(mar=c(5, 4, 5, 9))
palette(rainbow(9))
plot(FCbiomarkers.pca$rotation[, 1], FCbiomarkers.pca$rotation[, 3],
     col = Metadatavorinostattissue, pch = 19, xlab = "PC1", ylab = "PC2",
