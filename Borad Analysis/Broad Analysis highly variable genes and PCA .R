...
### for coloring ###
drug <- as.vector(Metadata$drug)
Treated.names <- rbind(Treated, drug)
drug <- as.factor(Treated.names[13300,])
  
palette(rainbow(15))
boxplot(Treated, medcol="black", border = NA, col= Metadata$drug,colnames=F,
        xlab="drugs", ylab="gene expression", main= "gene expression over treated cellines", las=3)
drug <- as.factor(levels(drug))
legend("topright", inset = c(-0.2,0), levels(drug), xpd = TRUE, pch=19, col = drug)

#### in funktionsbeschreibung suchen 
...
#### daten scalen !!! ??ber density ausprobieren
...
plot1U <- VariableFeaturePlot(BroadAnU)
#plot1U
plot2U <- LabelPoints(plot = plot1, points = top10Untreated, repel = TRUE, xnudge = 0, ynudge = 0)
#plot2U
...
# plot variable features with and without labels
plot1T <- VariableFeaturePlot(BroadAnT)
#plot1T
plot2T <- LabelPoints(plot = plot1, points = top10Treated, repel = TRUE, xnudge = 0, ynudge = 0)
plot2T 
...
# further Analysis --> which pathways --> via KEGG?
