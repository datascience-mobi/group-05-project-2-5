...
Therefore, we focused on data that reports different cellular responses to drug perturbations in cancer treatment. 
... 
...
palette(rainbow(15))
# Boxplot 
boxplot(Treated, medcol="black", border = drug, col= drug, xlab="drugs", ylab="gene expression",
...
legend("topright", inset = c(-0.28,0), legend= levels(drug), xpd = TRUE, pch=19, col = levels, title = "drugs")
```

The different colors indicate the 15 different anticancer drugs. We can can identify clearly 15 different boxes, each box belonging to one medicine. This indicates that we have batches between all 15 drugs. One reason for this could be, that the experiments were done on different days under slightly different conditions like air pressure or room temperature. Moreover, the black colored median shows us that the data is already normalized, because the medians of the single samples within one drug are at a similar height. 
...
# creat legend 
levels <- as.factor(levels(drug))
legend("topright", inset = c(-0.05,0.1), legend= levels(drug), xpd = TRUE, pch=19, col = levels, title = "drugs")
```
...
drug <- Metadata$drug
levels <- as.factor(levels(drug))
legend("topright", inset = c(-0.15,0), legend= levels(drug), xpd = TRUE, pch=19, col = levels, title = "drugs")
```
