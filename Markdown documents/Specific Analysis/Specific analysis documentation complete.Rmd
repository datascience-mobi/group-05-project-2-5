---
title: "Specific Analysis Documentation"
...
###Density plot: Comparison of gene expression with and without vorinostat treatment:
```{r,fig.height= 5, fig.width= 7}
...
First, we work with rowmeans to get a general overview which genes are generally up- or downregulated.
...
Now we can compare the gene vectors:

Do we have exactly the same biomarkers?
```{r}
setequal(biomarkergenes_noabs,biomarkers.genes)
```

How many of the biomarkers are different?
```{r}
diff= setdiff(biomarkergenes_noabs,biomarkers.genes)
length(diff)
```
...
head(pV_FC)
```
Now we know that p.Value and FC value do not deliver the same biomarkers. In fact, 85 out of 100 biomarkers are different.
If we compare the values directly, we see that a low p.value does not correlate with a high FC value. This is because small values are also assigned small p.values by the T-test and the result of the test is therefore falsified. For this reason, we cannot trust the biomarkers from the two-sided T-test and will now determine them via the __fold change__ values.To avoid, that up- or down-regulating effects cancel each other out, we will continue our work with those genes which show the highest absolute fold changes.
...
Now we have determined trusting biomarkers via the fold change. However, since we have worked only with absolute values, we cannot say whether our genes were up- or down-regulated.

Now we want to compare the biomarkers with and without using absolute values to determine how much they differ:
```{r}
...
We visualized the __FC__ of our biomarkers as well:
...
```
We see that most values are very similar to each other and that we have approximately the same number of genes regulated up and down.


__Visualization of biomarkers with Vulcanoplot:__
...
##PCA with general-biomarker-matrix:
...
##Find biomarkers only for leukemia:
...
legend(x = 1, y = 1, legend = c("Untreated", "Treated"), bty = "n",
       lty = 1, col = c("black", "red"), x.intersp = 0.5)
```
