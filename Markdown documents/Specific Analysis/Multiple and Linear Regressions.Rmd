...
```{r}
qqnorm(reg1$residuals, col = "royalblue2")
...
boxplot(lm_tab2$vorinostat, 
        main="Drug sensitivity", 
        sub=paste("Outlier rows: ", boxplot.stats(lm_tab2$vorinostat)$out)
        ) 
boxplot(lm_tab2$BM_Copynumber_meancol, 
        main="Copynumber", 
        sub=paste("Outlier rows: ", boxplot.stats(lm_tab2$BM_Copynumber_meancol)$out)
        ) 
```
...
```{r}
qqnorm(reg2$residuals, col = "springgreen4")
...
boxplot(lm_tab_all2$vorinostat, 
        main="Drug sensitivity", 
        sub=paste("Outlier rows: ", boxplot.stats(lm_tab_all2$vorinostat)$out)
        ) 
boxplot(lm_tab_all2$CN_meancol, 
        main="Copynumber", 
        sub=paste("Outlier rows: ", boxplot.stats(lm_tab_all2$CN_meancol)$out)
        ) 
```
...
```{r}
qqnorm(reg_all2$residuals, col = "royalblue1")
...
scatterplot3d(lm_tab_m)
```


# (0) Matrix with scatter plots, density plots and correlation values
...
ggpairs(data=lm_tab_m, columns=1:3, title="Multiple Regression")
```
...
boxplot(lm_tab_m$Drug_Sensitivity, 
        main="Drug sensitivity", 
        sub=paste("Outlier rows: ", boxplot.stats(lm_tab_m$Drug_Sensitivity)$out)
        ) 
boxplot(lm_tab_m$Doubling_Time, 
        main="Doubling time", 
        sub=paste("Outlier rows: ", boxplot.stats(lm_tab_m$Doubling_Time)$out)
        ) 
boxplot(lm_tab_m$Copynumber, 
        main="Copynumber", 
        sub=paste("Outlier rows: ", boxplot.stats(lm_tab_m$Copynumber)$out)) 
```
...
Skewness of the plot on the right:: -0.29 -> Plot is slightly skewed to the left.
...
This indicates that only 5.949% percent of the variation in the data (drug sensitivity)
...
```{r}
qqnorm(reg_m$residuals, col = "orchid1")
...
####  9.  SIMPLE LINEAR REGRESSION WITH ALL GENES: Drug sensitivity with doubling time                                      ####
