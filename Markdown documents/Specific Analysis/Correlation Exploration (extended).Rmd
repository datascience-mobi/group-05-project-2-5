...
This is how the heatmap looks making any modifications.
```{r}
pheatmap(cor1.2_tab, fontsize_row= 2.7, fontsize_col = 6)
```
...
pheatmap(cor1.2_tab_norm, fontsize_row= 2.7, fontsize_col = 6)
```
...
par(mar = rep(0,4))
circlize_dendrogram(d_pd30, labels_track_height = NA, dend_track_height = .3) 
```
...
par(mar = c(2,1,1,7))
plot(cor1_ret_hclust$tree_row, horiz  = TRUE, main = "Heatmap hierarchical clustering retrieved")
```
...
Strangely we dont see a correlation value of one between the same cell lines. We also see that different patterns of clustering for the cell lines emerge when we consider 100 biomarkers and when we consider 30 biomarkers. 
